...
from torch.distributed._shard.checkpoint import (
    FileSystemReader,
    FileSystemWriter,
    load as load_state_dict,
    save as save_state_dict,
)
...
class Checkpointer:
    ...
    def _write(self, state_dict, loader_state, process_group, save_name, rank):
        ...
        if loader_state is not None:
            torch.save(loader_state, os.path.join(save_name, f"loader_rank_{rank}.pth"))
            ...
            if os.path.isfile(load_path):
                ...
                if dataloader is not None:
                    ...
                    dataloader.load_state_dict(
                        torch.load(os.path.join(load_path, f"loader_rank_{self.rank}.pth"), weights_only=True)
                    )
    ...
    def save(
        ...
        **kwargs,
        ...
        dataloader_state = None if dataloader is None else dataloader.state_dict()
