...
from fms_fsdp.models.llama import LLaMA, LLaMABlock, param_init_function
...
def main(**kwargs):
    ...
    llama_config.attn_alpha_init_value = cfg.attn_alpha_init_value
    llama_config.ffn_alpha_init_value = cfg.ffn_alpha_init_value
    llama_config.dec_alpha_init_value = cfg.dec_alpha_init_value
    if rank == 0:
    ...
    dataset = DistributedDataset(cfg.data_path, rank, world_size, cfg.batch_size, cfg.seq_length, cfg.bos_token, cfg.eos_token)
    ...
    model = FSDP(
        ...
        param_init_fn=param_init_function if cfg.low_cpu_fsdp else None,
    )
    if rank == 0:
        print(model)
    ...
    model, optimizer, train_loader, start_step, tokens_seen, is_resuming = checkpointer.load(
        model,
        optimizer,
        train_loader,
